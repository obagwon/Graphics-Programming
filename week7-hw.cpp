...
public:
	...
	GLuint compile_shaders(void) {
		...
		vertex_shader = sb7::shader::load("week7-hw_vs.glsl", GL_VERTEX_SHADER);

		fragment_shader = sb7::shader::load("week7-hw_fs.glsl", GL_FRAGMENT_SHADER);
	...
	virtual void startup() {
		...
		// 텍스처
		glGenTextures(1, &texCon);
		glBindTexture(GL_TEXTURE_2D, texCon);
	...
	virtual void render(double currentTime) {
		...
		// 이동
		vmath::mat4 tm = vmath::translate((float)sin(currentTime) * 1.5f, 0.0f, (float)cos(currentTime) * 1.5f);
		...
		// 텍스처 바인드
		glUniform1i(glGetUniformLocation(rendering_program, "texture2"), 1);
